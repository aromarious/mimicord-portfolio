...
### 単体テスト (Unit Test) について

ビジネスロジックやコンポーネントの動作を検証する単体テストは、**特定の環境を必要としません**（環境非依存）。
外部依存（データベースや外部API）はすべてモック化されるため、この開発環境内で随時実行可能です。

- **ツール**: Vitest
- **実行コマンド**: `pnpm test:unit`
- **特徴**: `SKIP_ENV_VALIDATION=true` で実行され、環境変数やDB接続を一切必要としません。
