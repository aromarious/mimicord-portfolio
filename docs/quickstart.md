...
このプロジェクトでは **pnpm** (v10+) と **Infisical** を使用したローカル開発フローを採用しています。

## 1. 前提条件 (Prerequisites)

以下のツールがインストールされている必要があります。

* **Node.js**: v20以上 (Corepack enabled)
* **pnpm**: v10.11.0 以上
* **Docker Desktop** (または OrbStack 等): PostgreSQL (pgvector) 実行用
...
GitHub OAuth Client ID/Secret などの機密情報はすべて Infisical で管理されています。
...
プロジェクトには Docker Compose 設定が含まれており、以下のコマンドで **pgvector** 対応の PostgreSQL コンテナを起動できます。
...
4. **(Optional) データの投入**:
   RAG 機能（過去ログの検索・要約）をテストするには、データベースにメッセージデータが必要です。
   管理者が用意したデータインポートスクリプト (`apps/web/scripts/rag/ingest.ts`) を使用するか、開発用データセットについて管理者に問い合わせてください。
...
* **Frontend**: http://localhost:3000
  * GitHub Login をテストするには、ログイン画面から "Continue with GitHub" をクリックしてください。
  * ローカル環境(`localhost:3000`)用のコールバックURL設定済みアプリが使用されます。
...
Frontend (Next.js) から Backend API (Hono) を利用する方法や、認証機能の使用方法については、以下のガイドを参照してください。

* **[クライアント使用ガイド](./client_usage.md)**: `createClient` の使い方、公開/保護APIの呼び出し、認証機能の利用方法について解説しています。
...
### RAG検索が動作しない ("vector" type error)

`pgvector` 拡張機能が有効になっていない可能性があります。
`packages/config/docker-compose.yml` で `image: pgvector/pgvector:pg17` が使用されていることを確認し、コンテナを再構築（`docker compose down -v` してから `pnpm db:up`）してください。
