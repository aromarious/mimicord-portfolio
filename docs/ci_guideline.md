...
#### 2. Main Push時の完全テスト・デプロイ (`.github/workflows/full-test-deploy.yml`)

`main`ブランチへのプッシュ時に自動実行されます。PR時のテストに加えて、E2Eテストと本番デプロイも実行されます。
...
| **Deploy to Vercel**         | Vercel Production環境へのデプロイ             |    -     |
...
| 変数名                     | 取得方法                  | 用途                             |
| :------------------------- | :------------------------ | :------------------------------- |
| `DATABASE_URL`             | Infisical (`staging`環境) | PostgreSQL (pgvector) 接続文字列 |
| `OPENAI_API_KEY`           | Infisical (`staging`環境) | OpenAI API Key (RAG/Test用)      |
| `BETTER_AUTH_SECRET`       | Infisical (`staging`環境) | Better Auth 署名用シークレット   |
| `GITHUB_CLIENT_ID`         | Infisical (`staging`環境) | GitHub OAuth Client ID           |
| `GITHUB_CLIENT_SECRET`     | Infisical (`staging`環境) | GitHub OAuth Client Secret       |
...
- `DATABASE_URL` (pgvector対応のPostgreSQL)
- `OPENAI_API_KEY`
- `BETTER_AUTH_SECRET`
- `GITHUB_CLIENT_ID`
