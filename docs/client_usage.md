...
> [!NOTE]
> 認証機能はBetter Authを使用しており、現在はGitHub OAuthに対応しています。
...
- `signIn.social`: ソーシャルログイン (GitHub OAuth)
- `signOut`: サインアウト処理
...
await protectedClient.auth.signIn.social({
  provider: "github",
  callbackURL: "/dashboard" // サインイン後のリダイレクト先
})
```
...
- [API 設計ガイドライン](api_design_guidelines.md) - クライアント使用例を含むAPI設計の原則
- [認証フロー](auth-flow.md) - Better Auth認証の実装詳細
- [Hono API 実装標準](server_standards.md) - サーバーサイド実装標準
