...
* **例外 (RPCスタイル)**: 認証やステート変更など、リソースのCRUDで表現しにくい場合は動詞の使用を許可します。
  * 例: `/api/v0/protected/posts/:id/publish`
...
* 認証プロバイダーには **Better Auth** を使用しています。

## 6. クライアントからの呼び出し (Hono RPC)

`apps/web/lib/client.ts` が提供する型安全なRPCクライアントを使用します。
Better Auth のクライアントも `protectedClient.auth` に統合されています。
...
// 設計指針: 認証不要なリソースは publicClient からアクセス
const res = await publicClient.hello.$get() 
const data = await res.json()

// --- Protected API の呼び出しイメージ ---
// 設計指針: 認証が必要なリソースは protectedClient からアクセス
const postRes = await protectedClient.posts.$post({
  json: { title: "My Post", content: "..." }
})

// --- 認証機能 (Better Auth) の利用イメージ ---
// 認証機能も protectedClient.auth に統合されており、ここからアクセスします
...
> [!TIP]
> 実装詳細については [`apps/web/lib/client.ts`](../apps/web/lib/client.ts) を参照してください。
