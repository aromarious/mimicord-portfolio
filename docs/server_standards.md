...
> [!TIP]
> クライアント側での `AppType` の利用方法については [クライアント使用ガイド](./client_usage.md) を参照してください。
...
│   └── services/     # External API Adapters (OpenAI, etc.)
...
## 11. 認証ミドルウェア (Better Auth)

本プロジェクトでは **Better Auth** を採用しており、認証が必要なエンドポイントには専用のミドルウェアを適用します。
...
export const postsRoutes = new OpenAPIHono()
  .use("/protected/*", requireAuth) // protected配下すべてに適用
  .openapi(createPostRoute, async (c) => {
    const session = c.get("session") // 型推論される
    // ...
  })
```
...
OpenAI, Stripe, SendGrid などの外部サードパーティサービスと連携する際の設計・実装ルールです。

### 命名規則

- **Port (Interface)**
  - `apps/web/server/domain/ports/` に配置します。
  - **ルール**: 技術詳細（プロバイダー名など）を含まない、**機能や役割を表す抽象的な名前**とします。
    - ✅ `AIService`
    - ✅ `PaymentGateway`
    - ❌ `OpenAIService` (実装詳細が漏れている)
  - インターフェースであるため、`I` プレフィックスは付けません。

- **Implementation (Infrastructure)**
  - `apps/web/server/infrastructure/services/` に配置します。
  - **ルール**: **具体的なサービス名**を含め、実装内容が明確になる名前とします。末尾に `Impl` を付与して区別することもあります。
    - ✅ `OpenAIAIServiceImpl` (OpenAIを使用したAIServiceの実装)
    - ✅ `StripePaymentGateway`

- **Entity / Type**
  - **ルール**: 外部SDKの型定義を活用する場合でも、ドメイン層ではラッパーやエイリアスとして定義し、直接外部パッケージの型に依存する箇所を一元管理します。
  - **例外**: ユーザー指示により、外部パッケージの型をそのまま利用する方針とされた場合は、その型をドメイン層のインターフェース契約の一部として許容します。ただし、その場合でも `import type` を使用し、型としての依存に留めることが望ましいです。

### 実装方針

- **外部型（SDK型）の利用**
  - 独自の型定義とSDKの型定義を二重管理するコストを避けるため、**SDKが提供する型定義を積極的に活用**します。
  - 可能な限り、ドメイン層で定義するEntityやInterfaceの引数・戻り値として、SDKの型をラップまたはエイリアスして使用します。

### ディレクトリ構成例

```text
apps/web/server/domain/ports/
└── ai.service.ts          # interface AIService

apps/web/server/infrastructure/services/
└── openai.service.ts      # class OpenAIServiceImpl implements AIService
```

### RAG Engine の実装例

RAG (Retrieval-Augmented Generation) のような複雑なワークフローは `apps/web/lib/rag/` に配置します。
これは特定のドメインロジックではなく、横断的なユーティリティとして扱います。

```typescript
// apps/web/lib/rag/engine.ts
export class RagEngine {
  ...
  async search(query: string) { /* Vector search */ }
  async summarize(context: string) { /* LLM summarization */ }
}
```

## 13. ロギング標準

`packages/logger` を使用して構造化ログを出力します。
