...
INFISICAL_BIN="$ROOT_DIR/node_modules/.bin/infisical"

if [ ! -x "$INFISICAL_BIN" ]; then
  echo "Error: infisical CLI is not installed in the workspace."
...
for ENV in "${ENVIRONMENTS[@]}"; do
  ...
  if ! "$INFISICAL_BIN" export --projectId "$SOURCE_PROJECT_ID" --env "$ENV" --format json > "$EXORT_FILE"; then
  ...
  for KEY in $KEYS; do
      ...
      "$INFISICAL_BIN" secrets set "$KEY=@$VAL_FILE" --projectId "$DEST_PROJECT_ID" --env "$ENV" --silent
