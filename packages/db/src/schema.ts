...
export const PostTable = pgTable(
  ...
  (table) => ({
    userIdCreatedAtIdx: index("posts_user_id_created_at_idx").on(
      table.userId,
      table.createdAt,
    ),
  }),
...
export const DiscordMessageTable = pgTable(
  "discord_messages",
  (t) => ({
    id: t.bigserial("id", { mode: "number" }).primaryKey(),
    chunkId: t.text("chunk_id").notNull().unique(),
    content: t.text("content").notNull(),
    embedding: t.vector("embedding", { dimensions: 1536 }),
    messageDate: t.timestamp("message_date", { withTimezone: true }),
    createdAt: t.timestamp("created_at", { withTimezone: true }).defaultNow(),
  }),
  (table) => ({
    embeddingIdx: index("discord_messages_embedding_idx").using(
      "hnsw",
...
export type DiscordMessage = typeof DiscordMessageTable.$inferSelect
export type NewDiscordMessage = typeof DiscordMessageTable.$inferInsert
